package com.libGdx.tools.unplist.plist;
...
public class PlistAtlasOtherDemo implements Disposable {
		...
		public PlistAtlasData(FileHandle packFile, FileHandle imagesDir,TextureFilter minFilter,TextureFilter maxFilter) {
			try {
				...
				for (int i = 0; i < regionscount; i += 2) {
					...
					String text1 = value.getChild(1).getText();   //开始坐标 结尾坐标
					String text3 = value.getChild(5).getName();  //是不是发生旋转
					String text4 = value.getChild(7).getText();  // 原始宽高
...
//					}
	...
	public PlistAtlasOtherDemo() {
	}

	public PlistAtlasOtherDemo(String internalPackFile) {
		this(Gdx.files.internal(internalPackFile));
	}

	public PlistAtlasOtherDemo(FileHandle packFile) {
		this(packFile, packFile.parent());
	}

	public PlistAtlasOtherDemo(FileHandle packFile, FileHandle imagesDir) {
		this(new PlistAtlasData(packFile, imagesDir));
	}

	public PlistAtlasOtherDemo(PlistAtlasData data) {
