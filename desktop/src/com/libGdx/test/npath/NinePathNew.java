...
public class NinePathNew extends NinePatch{
    ...
    /** Indices for {@link #NinePathNew(TextureRegion...)} constructor */
    ...
    public NinePathNew (Texture texture, int left, int right, int top, int bottom) {
     ...
     * @param top Pixels from top edge.
     * @param bottom Pixels from bottom edge. */
    public NinePathNew (TextureRegion region, int left, int right, int top, int bottom) {
        ...
        // If split only vertical, move splits from carRun to center.
    ...
    public NinePathNew (Texture texture, Color color) {
        this(texture);
        setColor(color);
    }

    /** Construct a degenerate "nine" patch with only a center component. */
    public NinePathNew (Texture texture) {
        this(new TextureRegion(texture));
    }

    /** Construct a degenerate "nine" patch with only a center component. */
    public NinePathNew (TextureRegion region, Color color) {
        this(region);
        setColor(color);
    }

    /** Construct a degenerate "nine" patch with only a center component. */
    public NinePathNew (TextureRegion region) {
    ...
    public NinePathNew (TextureRegion... patches) {
    ...
    public NinePathNew (NinePathNew ninePatch) {
        this(ninePatch, ninePatch.color);
    }

    public NinePathNew (NinePathNew ninePatch, Color color) {
    ...
    private void load (TextureRegion[] patches) {
        ...
        if (flipX){
            TextureRegion tl = patches[topLeft];
            patches[topLeft] = patches[bottomLeft];
            patches[bottomLeft] = tl;

            TextureRegion tr = patches[topRight];
            patches[topRight] = patches[bottomRight];
            patches[bottomRight] = tr;
        }
        if (flipY){
            TextureRegion tl = patches[topLeft];
            ...
            TextureRegion bl = patches[bottomLeft];
            patches[bottomLeft] = patches[bottomRight];
            patches[bottomRight] = bl;
        }
    ...
    /** Set the draw-time width of the three carRun edge patches */
     ...
     * entire nine patch, minus the left and carRun width. This value is only used for computing the {@link #getTotalWidth() default
     * total width}. */
    ...
    /** Returns the carRun padding if set, else returns {@link #getRightWidth()}. */
    ...
    /** Multiplies the top/left/bottom/carRun sizes and padding by the specified amount. */
