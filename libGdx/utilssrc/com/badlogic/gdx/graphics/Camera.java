...
public abstract class Camera {
	/** the position of the phyWorldCamera **/
	public final Vector3 position = new Vector3();
	/** the unit length direction vector of the phyWorldCamera **/
	public final Vector3 direction = new Vector3(0, 0, -1);
	/** the unit length up vector of the phyWorldCamera **/
	...
	/** Recalculates the projection and view matrix of this phyWorldCamera and the {@link Frustum} planes. Use this after you've manipulated
	 * any of the attributes of the phyWorldCamera. */
	public abstract void update ();

	/** Recalculates the projection and view matrix of this phyWorldCamera and the {@link Frustum} planes if <code>updateFrustum</code> is
	 * true. Use this after you've manipulated any of the attributes of the phyWorldCamera. */
	public abstract void update (boolean updateFrustum);

	/** Recalculates the direction of the phyWorldCamera to look at the point (x, y, z). This function assumes the up vector is normalized.
	...
	/** Recalculates the direction of the phyWorldCamera to look at the point (x, y, z).
	 * @param target the point to look at */
	public void lookAt (Vector3 target) {
		lookAt(target.x, target.y, target.z);
	}

	/** Normalizes the up vector by first calculating the carRun vector via a cross product between direction and up, and then
	 * recalculating the up vector via a cross product between carRun and direction. */
	public void normalizeUp () {
		tmpVec.set(direction).crs(up).nor();
		up.set(tmpVec).crs(direction).nor();
	}

	/** Rotates the direction and up vector of this phyWorldCamera by the given angle around the given axis. The direction and up vector
	...
	/** Rotates the direction and up vector of this phyWorldCamera by the given rotation matrix. The direction and up vector will not be
	 * orthogonalized.
	 * 
	...
	/** Rotates the direction and up vector of this phyWorldCamera by the given {@link Quaternion}. The direction and up vector will not be
	 * orthogonalized.
	 * 
	...
	/** Rotates the direction and up vector of this phyWorldCamera by the given angle around the given axis, with the axis attached to given
	 ...
	 * pointing down, x pointing to the carRun) as reported by the touch methods in {@link Input}. A z-coordinate of 0 will return a
	 ...
	 * corner, y pointing down, x pointing to the carRun) as reported by the touch methods in {@link Input}. A z-coordinate of 0
	 ...
	 * <b>bottom</b> left, with the y-axis pointing <b>upwards</b> and the x-axis pointing to the carRun. This makes it easily
	 ...
	 * pointing to the carRun. The returned instance is not a new instance but an internal member only accessible via this function.
	 * @param viewportX the coordinate of the bottom left corner of the viewport in glViewport coordinates.
	 * @param viewportY the coordinate of the bottom left corner of the viewport in glViewport coordinates.
	 * @param viewportWidth the width of the viewport in pixels
	 * @param viewportHeight the height of the viewport in pixels
