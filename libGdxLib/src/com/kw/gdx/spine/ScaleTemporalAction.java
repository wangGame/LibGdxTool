...
//       其实没有必要这样写
        target.setScale(baseX + (currentX - startX),baseY + (currentY-startY));
        System.out.println(target.getScaleX() + "  ---   "+ target.getScaleY());
    }

    public void setCurvesX(float cx1, float cx2, float cx3, float cx4){
        setBezier(curvesX,startX,cx1,cx2,cx3,cx4,getDuration(),endX);
    }

    public void setCurvesY(float cx1,float cx2,float cx3,float cx4){
        setBezier(curvesY,startY,cx1,cx2,cx3,cx4,getDuration(),endY);
    }
